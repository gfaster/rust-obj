...
pub fn pixel_difference(
    ...
    for (x, y, pixel) in gray_img1.enumerate_pixels() {
        ret.put_pixel(
            x,
            y,
            pixel.map(|v| v.abs_diff(gray_img2.get_pixel(x, y).channels()[0])),
        );
    }
...
pub fn spectral_loss(
    ...
    let mut img1_buffer = gray_img1
        .as_raw()
        .iter()
        .map(|&pix| Complex::new(pix as f64 / 255.0, 0.0))
        .collect::<Vec<_>>();
    let mut img2_buffer = gray_img2
        .as_raw()
        .iter()
        .map(|&pix| Complex::new(pix as f64 / 255.0, 0.0))
        .collect::<Vec<_>>();
    ...
    for idx in 0..spec_loss.len() {
        ret.put_pixel(
            u32::try_from(idx).unwrap() % width,
            u32::try_from(idx).unwrap() % height,
            image::Luma([(spec_loss[idx] * 255.0) as u8]),
        );
    }
