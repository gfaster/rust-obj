...
fn main() {
    ...
    reads(std::env::args().nth(2).and_then(|a| a.parse().ok()).unwrap_or(9)).unwrap();
...
fn reads(res_power: i32) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for input in file.lines() {
        let obj = wavefrontobj::load(input).expect("pass a valid file path");
        screenshots_compare(obj, res_power);
    }
    Ok(())
}
...
fn cmp_fibsphere(cnt: usize) -> impl Iterator<Item = [glm::Vec3; 2]> {
    ...
    (0..cnt).map(move |_| {
        let r = (1.0 - z * z).sqrt();
        let out = [
            glm::vec3(long.cos() * 3.0 * r, z, long.sin() * 3.0 * r),
            glm::vec3((long + diff).cos() * 3.0 * r, z, (long + diff).sin() * 3.0 * r),
        ];
        // dbg!(&out);
        z -= dz;
        long -= dlong;
